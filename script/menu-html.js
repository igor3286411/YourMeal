...
const renderDish = (name, price, weight, img) => {
...
hamburgers.forEach(item => renderDish(item.name, item.price, item.weight, item.src))
...
nav.addEventListener('click', (event) => {
    ...
    if (click === buttonMenu[1]) {
        ...
        menu.textContent = ''
        snacks.forEach(item => renderDish(item.name, item.price, item.weight, item.src))
    }
    if (click === buttonMenu[2]) {
        nameMenu.textContent = 'Хот-доги'
        menu.textContent = ''
        hotDogs.forEach(item => renderDish(item.name, item.price, item.weight, item.src))
    }
})
